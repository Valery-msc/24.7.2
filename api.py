...
class PetFriends:
    ...
    def request(self, method: str, path: str, headers: str = '', data = None, params = None) -> tuple:
        """
        Send a request with authentication data (if exists) and returns the results
    ...
    def get_api_key(self,
        ...
        email = email or settings.valid_email
        password = password or settings.valid_password

        headers = {'email': email, 'password': password}
        auth_key = ''

        resp = self.sess.get(f'{self.base_url}api/key', headers=headers)
        if resp.status_code == 200:
            is_json = resp.headers['content-type'] == 'application/json'
            auth_key = resp.json()['key'] if is_json else resp.text
            self.headers['auth_key'] = auth_key
        return resp.status_code, auth_key


    def get_pets(self, pet_id=None, auth_key=None, filter: str = 'my_pets', status_code=None) -> list:
        """
