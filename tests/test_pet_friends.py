...
def test_get_api_key_for_valid_user():
    ...
    status, _ = pf.get_api_key(email=settings.valid_email,
                               password=settings.valid_password,
                               force=True)

    # Сверяем полученные данные с нашими ожиданиями
    assert status == 200
    assert type(_) is str and len(_) > 0

def test_get_api_key_for_invalid_user():
    """
    Получаем ключ auth_key для неверных данных (email, password)
    :param email: invalid_email
    :param password: invalid_password
    :return:
    """
    # Отправляем запрос и сохраняем полученный ответ с кодом статуса в status, а текст ответа в result
    status, _ = pf.get_api_key(email=settings.invalid_email,
...
def test_add_new_pet_with_valid_data(name='Барбоскин', animal_type='двортерьер', age='4', pet_photo='images/IMG_8496.jpeg'):
...
def test_update_pet(name='Мурзик', animal_type='Котэ', age=5):
    ...
    if len(my_pets) == 0:
        _, pet = pf.create_pet("Суперкот", "кот", "3", "images/IMG_8496.jpeg")
        my_pets = [pet]
...
def test_add_valid_photo(pet_photo='images/IMG_8496.jpeg'):
